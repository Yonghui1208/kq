...
function loadData() {
    let FirstTime;//第一次打卡的日期时间
    let LastTime;//最后一次打卡的日期时间
    let StartTime = 8 * 60 * 60 * 1000 + 50 * 60 * 1000 + 59 * 1000; //08：50：59
    let EndTime = 16 * 60 * 60 * 1000 + 50 * 60 * 1000;//16：50：00
    let date;
    let count = 0;
    let FTime;//第一次打卡的时间
    let LTime;//最后一次打卡的时间
    ...
    for (let i = 0; i < dataList.length; i++) {
        ...
        let Status = '';
        var datetime = new Date(dataList[i][3]);
        var Hours = parseInt(datetime.getHours());
        var Minutes = parseInt(datetime.getMinutes());
        var Seconds = parseInt(datetime.getSeconds());
        if (date != undefined) {
            //如果两个日期相等，说明当前这笔数据是当天的最后一次刷卡记录，只做赋值操作，
            //当读取下一个日期的数据时再用刚赋值的变量作比较，判断状态，并把相关变量重新赋值
            if (dateFormat(datetime) == dateFormat(new Date(date))) {
                FirstTime = Hours * 60 * 60 * 1000 + Minutes * 60 * 1000 + Seconds * 1000;
                FTime = add0(Hours) + ":" + add0(Minutes) + ":" + add0(Seconds);
                count++;
                //最后一笔数据
                if (i == dataList.length - 1) {
                    Status = newFunction();
                    console.log(dataList[i - count][0], dataList[i - count][1], dataList[i - count][2], dateFormat(new Date(date)), FTime, LTime, Status);
                }
            }
            else {
                if (count == 0) {
                ...
                //输出上一个日期的数据，所以要i-count
                console.log(dataList[i - count][0], dataList[i - count][1], dataList[i - count][2], dateFormat(new Date(date)), FTime, LTime, Status);
                //这是输出上一个日期的详细信息，所以判断放到输出的下面
                if (name != dataList[i][2]) {
                    console.log('');
                    console.log(`/Department  /EID  /Name  /Clock Time  /Status`);
                }
                //console.log(dataList[i][0], dataList[i][1], dataList[i][2], dateFormat(new Date(date)), FTime, LTime, Status);
                LastTime = Hours * 60 * 60 * 1000 + Minutes * 60 * 1000 + Seconds * 1000;
                LTime = add0(Hours) + ":" + add0(Minutes) + ":" + add0(Seconds);
                date = dataList[i][3];
                count = 0;
                name = dataList[i][2];
            }
    ...
    function newFunction() {
        ...
        if (LastTime - FirstTime < 9 * 60 * 60 * 1000) {
            Status += " 工时不足";
        }
        if (Status == "") {
            Status = " 正常";
